...
class CheckoutUserForm(forms.Form):
    ...
    type_of_delivery = forms.ChoiceField(required=False, widget=forms.RadioSelect(attrs={'class': 'radio-input', 'id': 'typeOfDelivery'}), choices=CHOICES)
    city_ref_hidden = forms.CharField(required=False, widget=forms.HiddenInput(attrs={'id': 'cityRefHidden'}))
    warehouse_id_hidden = forms.CharField(required=False, widget=forms.HiddenInput(attrs={'id': 'warehouseRefHidden'}))
